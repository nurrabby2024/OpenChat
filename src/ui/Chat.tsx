...
export function Chat(props: {
  ...
  return (
    <div className="flex-1 px-4 pb-3 pt-3 overflow-hidden">
      <div className="relative h-full rounded-2xl border border-line bg-[rgba(0,0,0,0.15)] overflow-hidden">
        ...
        {showNewChip ? (
          <button
            ...
            className="absolute left-1/2 -translate-x-1/2 bottom-36 rounded-full border border-line bg-[rgba(5,8,10,0.85)] backdrop-blur px-4 py-2 text-[11px] font-mono text-text shadow-glow hover:shadow-glow transition"
          >
            New messages
          </button>
        ) : null}
...
function MessageRow({ msg, mine, explorerBase }: { msg: ChatMessage; mine: boolean; explorerBase: string }) {
  ...
  const statusBadge =
    msg.status === "pending"
      ? <span className="inline-flex items-center gap-2 text-[11px] text-muted"><Spinner /> awaiting confirmation</span>
      : msg.status === "confirmed"
      ? <span className="inline-flex items-center gap-2 text-[11px] text-muted"><Check /> confirmed</span>
      : <span className="inline-flex items-center gap-2 text-[11px] text-err">failed</span>;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className={"font-mono text-[13px] leading-6 " + lineClass}>
...
function MsgText({ text }: { text: string }) {
  ...
  return (
    <span>
      ...
      {tooLong ? (
        <button
          className="ml-2 text-[11px] font-mono text-accent hover:underline underline-offset-4"
