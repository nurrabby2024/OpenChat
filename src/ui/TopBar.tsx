...
export function TopBar(props: {
  ...
  return (
    <div className="px-3 pt-3">
      <div className="rounded-2xl border border-line bg-panel backdrop-blur px-2.5 py-1.5 shadow-[0_0_0_1px_rgba(255,255,255,0.02)]">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden md:flex items-center gap-2">
              <button
                ...
                className="rounded-xl border border-line bg-[rgba(255,255,255,0.02)] px-2.5 py-1.5 text-[11px] font-mono text-muted hover:shadow-glow transition"
            ...
            >
              <span className="block max-w-[170px] sm:max-w-[240px] truncate whitespace-nowrap">{props.connectedLabel}</span>
            </button>
        ...
        >
          ...
          <div className="relative w-[min(720px,100%)] rounded-2xl border border-line bg-[rgba(5,8,10,0.92)] shadow-glow">
            <div className="px-2.5 py-1.5 border-b border-line flex items-center justify-between">
              <div className="text-[12px] font-mono text-text">Search</div>
              <button className="text-xs font-mono text-muted hover:text-text" onClick={() => setSearchOpen(false)}>Esc</button>
            </div>
            <div className="p-4">
              <div className="flex items-center gap-2 rounded-2xl border border-line bg-[rgba(255,255,255,0.02)] px-2.5 py-1.5">
                <span className="text-[12px] font-mono text-muted">/</span>
                <input
                  ref={searchRef}
                  value={filters.search}
                  onChange={(e) => setFilters({ ...filters, search: e.target.value })}
                  className="w-full bg-transparent outline-none text-[12px] font-mono text-text placeholder:text-muted"
...
function Toggle({ label, on, onToggle }: { label: string; on: boolean; onToggle: () => void }) {
  return (
    <button
      onClick={onToggle}
      className={[
        "rounded-xl border px-2.5 py-1.5 text-[11px] font-mono transition",
