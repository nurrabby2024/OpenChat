...
import { CHAT_CONTRACT, fetchContractShape, buildDataSuffix, encodeSendData, walletSendCalls, walletGetCallsStatus } from "../lib/contract";
...
function AppInner() {
  ...
  // listen account/chain changes (use the resolved provider; MetaMask shouldn't hijack Base preview)
  useEffect(() => {
  ...
  // Contract shape + builder suffix load (NO ABI required)
  useEffect(() => {
    let alive = true;
    const run = async (attempt = 0) => {
      try {
        setContractError("");
        const shape = await fetchContractShape();
        ...
        // Quiet retry a few times (cold start / explorer hiccup)
        if (attempt < 3) {
  ...
  // Initial messages load (via Blockscout interpreter, no ABI)
  useEffect(() => {
    let alive = true;
    ...
    const load = async () => {
      ...
      try {
        ...
        // Keep any local pending/failed messages already in state
        setMessages((prev) => {
    ...
    // Poll for new confirmed txs (keeps UI fresh)
  ...
  const sendMessage = async (text: string) => {
    if (!shapeReady) {
      toast.push({ title: contractError ? "Contract unavailable" : "Still loading contract", detail: contractError ? contractError : "Try again in a second.", kind: "warn" });
    ...
    try {
      ...
      toast.push({ title: "Failed — retry", detail: (String(msg).includes("wallet_sendCalls") || String(msg).toLowerCase().includes("method not found") || String(msg).includes("-32601")) ? "Your current wallet doesn\'t support wallet_sendCalls. Open this in the Base App / a Farcaster Mini App wallet environment to publish onchain." : msg, kind: "err" });
